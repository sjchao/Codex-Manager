...
| 平台密钥 | `service_apikey_create` | `apikey/create` | 参数：`name`、`modelSlug`、`reasoningEffort`、`serviceTier`、`protocolType`、`upstreamBaseUrl`、`staticHeadersJson`、`rotationStrategy`、`aggregateApiId`、`allowedModels`（为空表示不限制可用模型） |
| 平台密钥 | `service_apikey_models` | `apikey/models` | 参数可选：`refreshRemote` |
| 平台密钥 | `service_apikey_usage_stats` | `apikey/usageStats` | 用量统计 |
| 平台密钥 | `service_apikey_update_model` | `apikey/updateModel` | 参数：`keyId`，以及与创建相同的一组可更新字段；`allowedModels` 传空数组表示恢复不限制 |
...
| 请求日志 | `service_requestlog_list` | `requestlog/list` | 参数：`query`、`statusFilter`、`page`、`pageSize`、`keyName`（密钥名称精确匹配） |
| 请求日志 | `service_requestlog_summary` | `requestlog/summary` | 参数：`query`、`statusFilter`、`keyName`（密钥名称精确匹配） |
| 请求日志 | `service_requestlog_clear` | `requestlog/clear` | 清空请求日志 |
| 请求日志 | `service_requestlog_prune` | `requestlog/prune` | 删除一周前的请求日志，返回删除条数 |
| 请求日志 | `service_requestlog_today_summary` | `requestlog/today_summary` | 今日汇总 |
| 请求日志 | `service_requestlog_model_usage` | `requestlog/model_usage` | 今日各模型 Token 消耗（按模型聚合） |
