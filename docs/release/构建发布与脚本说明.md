...
### Linux Service 三件套
```bash
./scripts/build-linux-service-package.sh

# 本地快速增量打包（保留现有 apps/out）
./scripts/build-linux-service-package.sh --no-clean-dist

# 产物改到自定义目录
./scripts/build-linux-service-package.sh --output-dir dist/linux-service
```

- 无参数默认按“正式包”处理：会先清理 `apps/out`，再重新导出前端并构建三件套。
- 默认产物目录：`target/release/linux-package/`
- 默认产物文件：
  - `codexmanager-service`
  - `codexmanager-web`
  - `codexmanager-start`
- `codexmanager-web` 会固定以 `--features embedded-ui` 编译，把 `apps/out` 静态资源打进二进制，避免运行时依赖外部前端目录。
