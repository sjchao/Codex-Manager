...
fn init_tracks_schema_migrations_and_is_idempotent() {
    ...
    let applied_044: i64 = storage
        .conn
        .query_row(
            "SELECT COUNT(1) FROM schema_migrations WHERE version = '044_request_logs_aggregate_api_failure_chain'",
            [],
            |row| row.get(0),
        )
        .expect("count 044 migration");
    assert_eq!(applied_044, 1);
    let applied_045: i64 = storage
        .conn
        .query_row(
            "SELECT COUNT(1) FROM schema_migrations WHERE version = '045_aggregate_api_weight'",
            [],
            |row| row.get(0),
        )
    ...
    let applied_046: i64 = storage
        .conn
        .query_row(
            "SELECT COUNT(1) FROM schema_migrations WHERE version = '046_request_logs_queue_wait_ms'",
            [],
            |row| row.get(0),
        )
    ...
    let applied_049: i64 = storage
        .conn
        .query_row(
            "SELECT COUNT(1) FROM schema_migrations WHERE version = '049_api_keys_group_name'",
            [],
            |row| row.get(0),
        )
    ...
    assert!(storage
        .has_column("request_logs", "queue_wait_ms")
        .expect("check request_logs.queue_wait_ms"));
    assert!(storage
    ...
    assert!(storage
        .has_column("request_logs", "aggregate_api_attempt_failures_json")
        .expect("check request_logs.aggregate_api_attempt_failures_json"));
    assert!(storage
        .has_column("aggregate_apis", "weight")
        .expect("check aggregate_apis.weight"));
    assert!(storage
    ...
    assert!(storage
        .has_column("api_keys", "group_name")
        .expect("check api_keys.group_name"));
    assert!(!storage
